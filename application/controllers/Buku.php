...
class Buku extends CI_Controller
{
    ...
    {
        parent::__construct();
        cek_login(); // Ensure user is logged in
 ...
 Pengarang', 'required|min_length[3]', [
    ...
    public function kategori()
    {
        ...
        if ($this->form_validation->run() == false) {
            // Load views if validation fails
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            ...
            // Insert data
            $this->ModelBuku->simpanKategori($kategoriData);
    ...
    {
        // Use ID directly from function parameter instead of $this->uri->segment()
    ...
    public function ubahKategori()
    {
        ...
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['kategori'] = $this->ModelBuku->kategoriWhere(['id' => $this->uri->segment(3)])->result_array(); // Use row_array for single record
        ...
        if ($this->form_validation->run() == false) {
            ...
            $updatedData = [
                'kategori' => $this->input->post('kategori', true) // Sanitize input
            ];

            // Perform the update using the ID passed
            $this->ModelBuku->updateKategori(['id' => $this->input->post('id')], $updatedData);
